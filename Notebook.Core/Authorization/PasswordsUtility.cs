...
{
    internal class PasswordsUtility
    {
        ...
        public static async Task<bool> VerifyPasswordAsync(string passwordGuess, string passwordHash)
        {
            ...
            int iterationCountLength = actualSavedHashResultsBtyeArr.Length -
                (salt.Length + actualPasswordByteArr.Length);
            ...
            byte[] passwordGuessByteArr = GenerateHashValue(passwordGuess, salt,
                BitConverter.ToInt32(iterationCountByteArr, 0));

            return await ConstantTimeComparisonAsync(passwordGuessByteArr, actualPasswordByteArr);
        }

        private static async Task<bool> ConstantTimeComparisonAsync(byte[] passwordGuess, byte[] actualPassword)
        {
            return await Task.Run(() => ConstantTimeComparison(passwordGuess, actualPassword));
        }
